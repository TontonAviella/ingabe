...
GENERATED_SCHEMAS: Dict[str, Dict[str, Any]] = {
    ...
    'display_layer': {
        ...
        'parameters': {'type': 'object', 'properties': {'asset_url': {'description': "Public HTTPS URL of a Cloud-Optimized GeoTIFF (COG). Examples: 'https://isdasoil.s3.amazonaws.com/soil_data/nitrogen_total/nitrogen_total.tif', 'https://earth-search.aws.element84.com/...../B04.tif'. GeoJSON URLs and S3 protocol URLs are not yet supported in this version.", 'title': 'Asset Url', 'type': 'string'}, 'layer_name': {'description': "Human-readable layer name shown in the layers panel, e.g. 'Soil Nitrogen — Cyampirita'", 'title': 'Layer Name', 'type': 'string'}, 'style_hint': {'description': 'Style preset that picks the colormap and value range. One of: ndvi, ndwi, nbr, visual, ndvi_band, ndre_band, soil_nitrogen, soil_phosphorus, soil_potassium, soil_ph, soil_organic_carbon, soil_clay, soil_sand, anomaly_zscore, drought_severity, soil_moisture, evapotranspiration, temperature, rainfall, sar_backscatter_db.', 'title': 'Style Hint', 'type': 'string'}, 'bbox': {'description': "Bounding box of the area to display, as 'west,south,east,north' in WGS84. Used for auto-zoom.", 'title': 'Bbox', 'type': 'string'}, 'band_index': {'description': '1-based band index for single-band rasters (use 1 for most soil COGs and z-score rasters).', 'title': 'Band Index', 'type': 'integer'}}, 'required': ['asset_url', 'layer_name', 'style_hint', 'bbox', 'band_index']},
    },
    ...
    'render_map_snapshot': {
        ...
        'description': "Render a static PNG snapshot of the current map and dispatch it to a delivery channel.\n\n    WHEN TO USE: the user wants a snapshot they can SHARE outside the browser —\n    Telegram message, WhatsApp message, email attachment — or wants a permalink\n    PNG of the current view. Also use for cron-fired reports to insurance partners.\n\n    WHEN NOT TO USE: the user wants to keep working in the browser (they already\n    see the map). Numeric/tabular results — use the specific stats tool. Adding a\n    NEW data layer on the map — use display_satellite_layer or compute_spectral_index.\n\n    Delivery model: renders PNG, uploads to MinIO under snapshots/{map_id}/, then\n    publishes a payload on the 'mundi:render_snapshot' Redis channel. A separate\n    sender process consumes the channel, filters on delivery_channel and partner_id,\n    and performs the actual send. Returns once the payload is published; does NOT\n    block on the sender. Snapshot URLs are valid for 24 hours.",
        'parameters': {'type': 'object', 'properties': {'bbox': {'description': "Bounding box 'west,south,east,north' in WGS84, e.g. '29.44,-1.72,29.68,-1.50'", 'title': 'Bbox', 'type': 'string'}, 'width': {'description': 'Image width in pixels. Use 1024 for chat attachments, 1600 for reports.', 'title': 'Width', 'type': 'integer'}, 'height': {'description': 'Image height in pixels. Use 600 for chat, 1000 for reports.', 'title': 'Height', 'type': 'integer'}, 'caption': {'description': "One-line caption shown to the recipient, e.g. 'NDVI Cyampirita 2026-05-12, mean=0.62'.", 'title': 'Caption', 'type': 'string'}, 'delivery_channel': {'description': "Where to send the snapshot. 'browser' = the user's current map view. 'telegram'/'whatsapp'/'email' route to an out-of-band sender; recipient must be set for those.", 'enum': ['browser', 'telegram', 'whatsapp', 'email'], 'title': 'Delivery Channel', 'type': 'string'}, 'recipient': {'description': "Destination for non-browser channels: Telegram chat_id, WhatsApp E.164 number, or email address. Pass an empty string for 'browser'.", 'title': 'Recipient', 'type': 'string'}}, 'required': ['bbox', 'width', 'height', 'caption', 'delivery_channel', 'recipient']},
    },
    'zoom_to_bounds': {
        'name': 'zoom_to_bounds',
...
def _make_stub_handler(tool_name: str):
    """Stub handler returning a structured 'not yet wired' error.

    Returning args_received per tool call lets us see EXACTLY which
    Sage tools the LLM tries to call with what args. Reconnaissance
    for Phase 2 wiring priority.
    """
    def _stub(args, **kw):
        return json.dumps({
            'status': 'not_yet_wired_in_hermes',
            'tool_name': tool_name,
            'args_received': args,
            'task_id': kw.get('task_id'),
            'note': (
                'This Sage tool is REGISTERED with Hermes but not yet '
                'EXECUTABLE. Phase 2 of the Hermes migration ('
                'in-process integration with mundi.ai) wires the real '
                'handler. For now, Hermes can see the tool surface but '
                'cannot dispatch the actual geoprocessing.'
            ),
        })
    return _stub
