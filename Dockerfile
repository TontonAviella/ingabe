...
# LAStools – build from source to avoid GLIBC 2.38 requirement of pre-built binaries
FROM base AS lastools-builder
WORKDIR /tmp/las
RUN apt-get update && apt-get install -y --no-install-recommends \
        git build-essential cmake ca-certificates \
    && rm -rf /var/lib/apt/lists/*
ARG LASTOOLS_TAG=v2.0.4
RUN git clone --depth 1 --branch ${LASTOOLS_TAG} https://github.com/LAStools/LAStools.git /tmp/las/LAStools \
    && cd /tmp/las/LAStools \
    && mkdir build && cd build \
    && cmake .. -DCMAKE_BUILD_TYPE=Release \
    && make -j$(nproc) \
    && cp /tmp/las/LAStools/bin64/las2las64 /usr/local/bin/ \
    && chmod +x /usr/local/bin/las2las64 \
    && cp /tmp/las/LAStools/bin64/lasinfo64 /usr/local/bin/lasinfo64 \
