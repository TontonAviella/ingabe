import { TrendingDown, TrendingUp, Minus, MapPin, Activity, AlertTriangle } from 'lucide-react';
...
import {
  ...
  useMLStatus,
  useNdviTimeseries,
  useYieldRiskPrediction,
  type NdviTimeseriesPoint,
} from '@/hooks/useRwandaApi';
    ...
    <div className="flex flex-col gap-4 p-6 max-w-7xl mx-auto">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            H3 hexagonal grid showing NDVI vegetation health across Rwanda
          </CardDescription>
        </CardHeader>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  ...
                  <div className="p-4 bg-blue-50 dark:bg-blue-900/20 rounded-lg border border-blue-200 dark:border-blue-800">
                    ...
                    <div className="text-2xl font-bold text-blue-700 dark:text-blue-300">
                      {summary.avg_ndvi?.toFixed(3) ?? "N/A"}
                    </div>
                    <div className="text-xs text-blue-600 dark:text-blue-400 mt-1">
                      {getNdviHealthLabel(summary.avg_ndvi)}
                    </div>
                  </div>
                  ...
                  <div className="p-4 bg-amber-50 dark:bg-amber-900/20 rounded-lg border border-amber-200 dark:border-amber-800">
                    ...
                    {yieldRisk && !isAnalyzing && (
                      <div className="flex items-center gap-2">
                        <Badge variant={getRiskVariant(yieldRisk.risk_level)}>
                          {yieldRisk.risk_level.toUpperCase()}
                        </Badge>
                        <span className="text-xs text-muted-foreground">
                          {(yieldRisk.confidence * 100).toFixed(0)}% confidence
                        </span>
                      </div>
                    )}
                    {!yieldRisk && !isAnalyzing && timeseries && (
      ...
      {mlStatus && (
        <div className="flex items-center justify-center gap-2 text-xs text-muted-foreground">
          <span
            className={cn(
              "size-2 rounded-full",
              mlStatus.ml_ready ? "bg-green-500 animate-pulse" : "bg-yellow-500"
            )}
          />
          <span>
