...
async def download_from_openstreetmap(
    ...
    api_key = os.environ.get("INGABE_OSM_API_KEY")
    if not api_key:
    ...
    osm_url = f"https://osm.ingabe.com/v1/osm/extract?tags={tags}&api_key={api_key}&bbox={bbox_str}"
        ...
        for geom in geometry_types:
            ...
            if gpkg_path.exists() and gpkg_path.stat().st_size > 0:
                try:
                    ...
                    logger.debug("ogrinfo failed for feature count, defaulting to 0", exc_info=True)
                    feature_count = 0
