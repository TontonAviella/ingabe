...
def analyze_raster_object_candidates(payload: RasterObjectCandidateInput) -> dict[str, Any]:
    ...
    This is a first-pass candidate extractor, not a semantic foundation model.
    It is intentionally useful without torch/opencv/samgeo installed, and the
    returned engine metadata makes that limitation explicit.
    """
...
def _optional_engine_status() -> dict[str, dict[str, Any]]:
    return {
        ...
        "geolibre_rust_wasm": {
            "installed": False,
            "note": "Expected as browser/WASM package; not importable from this Python backend.",
        },
    }
