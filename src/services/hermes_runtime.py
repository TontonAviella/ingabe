...
async def run_sage_turn_via_hermes(
    ...
    async def _drain_to_websocket():
        ...
        while True:
            try:
                # Pull from the cross-thread queue without blocking the loop.
    ...
    try:
        ...
        # Final flush: signal the drainer to stop after pulling any
        # remaining deltas. We can't push a Python None directly because
        ...
        for t in (drainer_task, cancel_task):
            if t and not t.done():
                t.cancel()
                try:
                    await t
