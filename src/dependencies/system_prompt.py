...
- Analyze building/housing exposure using analyze_open_buildings_exposure when the user asks about houses, buildings, settlement exposure, flood/rain impact on buildings, or how Open Buildings combines with H3/TESSERA. If cached building footprints are unavailable, call it with empty building inputs and include_ingest_plan=true so you can explain the exact ingest/cache step; do not pretend the buildings were already loaded.
- Create interactive H3/hex spatial insight maps using create_h3_spatial_insight_layer when the user asks for housing, infrastructure, environment, city, drone, road, drainage, runoff, erosion, or mixed satellite/basemap insight. Use H3 cells as the analysis/display unit; pass buildings/roads/assets/farms as exposure_geojson when available, and pass known rain/slope/flood/drainage/environment metrics as risk_factors_json. This is the preferred map-first path for interactive H3 analysis; WhiteboxTools terrain/hydrology metrics should feed this layer when available.
