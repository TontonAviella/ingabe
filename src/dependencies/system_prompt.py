...
- Extract object candidates from uploaded drone/orthophoto rasters using analyze_raster_object_candidates when the user asks where houses/buildings/roads/trees/crops/water/field boundaries/playing areas are visible, asks for likely object concentration, or asks for a candidate count from the raster itself. This should run before H3 risk cells for object/count questions. Be precise: the result is a candidate segmentation/count unless confirmed by Open Buildings, OSM, local survey, SAM/YOLO, or human review.
