            ...
            if self.type == "postgis":
                ...
                try:
                    ...
                    if never_return_local_file:
                        ...
                        await s3_op(s3_client.upload_file(temp_gpkg_path, bucket_name, s3_key, Config=one_shot_config),
                                    "upload", f"PostGIS temp {s3_key}", raise_http=False)

                        presigned_url = await s3_op(
                            s3_client.generate_presigned_url("get_object", Params={"Bucket": bucket_name, "Key": s3_key}, ExpiresIn=900),
                            "presigned URL", f"PostGIS temp {s3_key}", raise_http=False,
                        )
                ...
                if never_return_local_file:
                    ...
                    try:
                        ...
                        from src.utils import s3_op
                        await s3_op(s3_client.download_file(bucket_name, self.s3_key, temp_path),
                                    "download", f"layer {self.layer_id}", raise_http=False)
                        yield temp_path
                    finally:
