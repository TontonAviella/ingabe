...
[open source Mundi](https://github.com/Ingabe/mundi.ai), which is free and self-hostable. We recommend on-prem Mundi only for large
...
information, [schedule a call](https://cal.com/ingabe/30min) with us.
:::
...
[open source Mundi](https://github.com/Ingabe/mundi.ai) and for on-prem Mundi cloud.
...
to the Ingabe team is not required.
...
cloud-hosted configurations, the [Ingabe team](https://ingabe.com/)
...
:::note
Ingabe-hosted Mundi cloud is not FedRAMP Authorized.
:::

Mundi can be deployed to federal users, even though we are not FedRAMP authorized. Because Ingabe is not FedRAMP authorized, deploying Mundi requires an on-premises deployment in a FedRAMP authorized cloud.
